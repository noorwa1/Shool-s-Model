...
class TeacherLog(Screen):
    ...
    def delete(self):
        firebase=Firebase.FirebaseApp('https://file-uploading-2e4d3-default-rtdb.firebaseio.com/')
        firebase.delete('/Message/')
    ...
    def selection(self, *args):
        ...
        storage = firebase.storage()
        storage.child(cloud_file_name).put(str(args[1][0]))
...
class Apeals_sending(Screen):
    ...
    def patch(self, JSON):
        data = {'Message': self.msgtxt, 'From': self.remail, 'To': self.teachermail,
                'Date': self.tnow.strftime("%d%m%y, %H:%M:%S")}
    ...
    def printtxt(self):
        ...
        db2.child('Message/Apeal').child(email.split("@")[0]).set(data)
